...
class RandomGen8Teams {
  ...
  randomSet(species, teamDetails = {}, isLead = false, isDoubles = false, isNoDynamax = false) {
    ...
    const randMoves = isDoubles && species.randomDoubleBattleMoves || isNoDynamax && species.randomBattleNoDynamaxMoves || species.randomBattleMoves;
  ...
  randomTeam() {
    ...
    while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (this.format.gameType === "singles") {
        ...
        if (!species.randomDoubleBattleMoves?.length)
          continue;
      }
